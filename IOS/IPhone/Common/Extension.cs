...
namespace HashBot
{
	public static class Extension
	{
		...
		public static void ApplyImageFromUrlAsunc (this UIImageView view, string weburl, UIImage defaultImage)
		{
			ThreadPool.QueueUserWorkItem ((state) =>	 {
				...
				var imgdata = NSData.FromUrl(new NSUrl(weburl));
				view.InvokeOnMainThread (() =>   {
					if (imgdata != null) {
						view.Image = new UIImage (imgdata);
					}
					else {
						view.Image = new UIImage ();
					}
					view.ApplyImageMask("Images/Main/mask_avatar.png");
				});
			});
